...
sub process_pic_ref {
  ...
  print "$file:$line: " . decode("utf8", "图片") . " ($pic_path) " . decode("utf8", "不存在"). "\n"
    and return 0
    if not -e encode("utf8", $pic_full_path);
  ...
  if ($pic_path ne $suggest_pic_path) {
    ...
    print "$file:$line: rename $pic_path ==> $suggest_pic_path fail, $!\n"
      and return 0 if not rename encode("utf8", $pic_full_path), encode("utf8", $suggest_pic_full_path);

    ${ $row } =~ s/\!\[([^]]*)\]\([^)]+\)/\![$1]($suggest_pic_path)/;
    $need_update = 1;

    my $suggest_pic_base_name = $suggest_pic_dir[$#suggest_pic_dir];
    $pic_repo{$suggest_pic_base_name} = $pic_data;
  }
