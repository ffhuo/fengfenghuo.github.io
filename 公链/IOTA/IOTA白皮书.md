...
仍然很难准确地求解方程（6），因此我们进一步进行简化假设。首先，我们可以看到，对于任意固定 $ε > 0$ , 在当 $K(t)$ 达到 $εL_0$ 水平之后， $K(t)$ 将会迅速增长到$L_0$ 。现在，当 $K(t)$ 相对于$L_0$ 非常小的时候，我们可以舍弃掉方程（6）右边最后一项。同时，用 ![K(t)](media/IOTA白皮书-K(t).png) 代替 $K(t-h)$ ，我们可以得到方程（6）的简化版（记住 $\frac{λh}{L_0} = ln2$):
![dK(t)](media/IOTA白皮书-dK(t).png)

其中边界条件为 $K(0) = 1$ 。求解上述微分方程可以得到
![K(t)微分方程](media/IOTA白皮书-K(t)微分方程.png)
...
![dH(t)dt](media/IOTA白皮书-dH(t)dt.png)

因此，
![H(t)](media/IOTA白皮书-H(t).png)
